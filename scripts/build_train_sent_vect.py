# -*- coding: utf-8 -*-
# --------------------------------------------------------------------
# MappSent: Build training sentence embedding vectors				 -
#                                                                    -
# Author  : Amir HAZEM                                               -     
# Created : 13/02/2017                                               -  
# Updated : 15/11/2017                                               -
...
# seed dictionary to build the training sentence mapping  matrix
fname_dict="../exp/vectsent/dico_sent_mapping.txt"
#----------------------------------------------------------------------------------------
...
sum_vect_embedding_sent_Q={}
sum_vect_embedding_sent_Q["word_Q"]=np.zeros(dim,float)
cpt_tgt_test=1
#----------------------------------------------------------------------------------------
...
# -------------------------------------------------------------
# Define and use Wordnet POSTAGS
# -------------------------------------------------------------
def is_noun(tag):
...
# -------------------------------------------------------------
# load_size_test: Compute size test data in number of sentences
# -------------------------------------------------------------
def load_size_test(cpt_tgt_test):
	...
	#print ('Found %s test sentences ' % (cpt_tgt_test))
	return cpt_tgt_test
...
# ------------------------------------------------------------
def save_source_sentences(vect_sent_src,vect_sent_tgt):
	fname_res="../exp/vectsent/vect_sent_src_train.txt"

	with  open(fname_res,'w') as fi:
		line=int(cpt_src/10 + cpt_tgt+ (cpt_tgt_test) + cpt_tgt_test/10)
		fi.write(str((line))+" "+str(dim)+"\n")
...
def save_target_sentences(vect_sent_tgt):
	fname_res="../exp/vectsent/vect_sent_tgt_train.txt"
...
# ------------------------------------------------------------
# parse_train: Parse data and build sentence embedding vectors 
# ------------------------------------------------------------
def parse_train():
	with  open(fname_dict,'w') as fi:
		...
		dico_chosen={}
		dico_chosenalready={}
		reader = unicodecsv.reader(open(input_file),delimiter='\t')
		next(reader)
		for line in reader:
			...
			if rank==1:
				...
				# Generate the mapping dictionary from positive training labels
				if label==1:
									
					if cpt_t==0:
						dico_chosen[cpt_t]=RELQ_ID
						tmp_ori=ORGQ_ID
						cpt_t+=1
					else:
						if tmp_ori==ORGQ_ID: 	
							...
							tmp_ori=ORGQ_ID
							for i in range(0,cpt_t):
								for j in range(i+1,cpt_t):
									
									resp= str(dico_chosen[i]) + str('\t')+ str(dico_chosen[j])				
					...
					if not dico_chosenalready.has_key(ORGQ_ID):
						dico_chosenalready[ORGQ_ID]=ORGQ_ID
						input_dico=ORGQ_ID+" "+ RELQ_ID
					
						resp=ORGQ_ID.encode('utf-8') + '\t'+ RELQ_ID.encode('utf-8')
						fi.write(resp.encode('utf-8')+"\n")   
	return(cpt_src,cpt_tgt)					
...
if __name__=='__main__':
	...
	# compute test size (needed for mapping)
	cpt_tgt_test=load_size_test(cpt_tgt_test)
	
	# compute sentence embeddings of the training data set and produce the seed dictionary needed for mapping
	(cpt_src,cpt_tgt)=parse_train()

	# save sentence embeddings 
	save_source_sentences(vect_sent_src,vect_sent_tgt)
